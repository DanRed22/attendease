...
import { getCookies, getUserRole } from '@/utils/auth'
...
export default function Settings() {
    ...
    const fetchUserRole = async () => {
        ...
        const current_user = await getCookies()
        setCurrentUser(JSON.parse(current_user?.user))
        setUserRole(role)
    }
    useEffect(() => {
        fetchUserRole()
    ...
    return (
        <div className="container mx-auto px-4 py-8 h-screen w-screen">
            ...
            {(userRole === 'ADMIN' || userRole === 'admin') && (
                <div className="flex justify-center flex-col mt-8 container w-full p-4 bg-slate-600 rounded-lg shadow-md">
                    ...
                    <p className="text-sm text-gray-300">
                        Add, edit, and delete fields. Note that duplicate
                        database name will not be added to the table.
                    </p>
            ...
            {userRole !== 'ADMIN' && (
                <p>Some features of this page is for admin users only.</p>
            )}
        </div>
    )
}
